    ...
    public partial class MainWindow : Window
    {
        ...
        stars.View = starGridView;
            ...
            foreach (string system in starSystemList)
            {
                ...
                stars.Items.Add(new SystemData() {systemName = system, stationName = station, distanceSystem = distantSys});


               // masterList.Add(new SystemData() { , distance={3.0}});
        ...
        private void PlotStations()
        {

            
            Canvas dotCanvasXZ = new Canvas();
            Canvas dotCanvasYZ = new Canvas();
            Canvas dotCanvasYX = new Canvas();
            Canvas dotCanvasZY = new Canvas();
         ...
         //   dot.originX = stageXZ.Width / 2;
        //    dot.originY = stageXZ.Height / 2;
            ...
            foreach (var loc in location)
            {
                ...
                if (loc.Key > stationA && loc.Key<stationB)
                {
                    ...
                    dotCanvasXZ.Margin = new Thickness(offset + (dot.x*scale), offset - (dot.y*scale), 0, 0);

                    dotCanvasXZ = dot.Draw(color, size);

              //      stageXZ.Children.Add(dotCanvasXZ);

                    dot.x = y;
                    dot.y = z;

                    dotCanvasYZ.Margin = new Thickness(offset + (dot.x * scale), offset - (dot.y * scale), 0, 0);

                    dotCanvasYZ = dot.Draw(color, size);

                //    stageYZ.Children.Add(dotCanvasYZ);

                    dot.x = y;
                    dot.y = x;

                    dotCanvasYX.Margin = new Thickness(offset + (dot.x * scale), offset - (dot.y * scale), 0, 0);

                    dotCanvasYX = dot.Draw(color, size);

              //      stageXY.Children.Add(dotCanvasYX);

                    dot.x = z;
                    dot.y = y;

                    dotCanvasZY.Margin = new Thickness(offset + (dot.x ), offset - (dot.y ), 0, 0);

                    dotCanvasZY = dot.Draw(color, size);

                   // stageZY.Children.Add(dotCanvasZY);
                }
            }
        }
        ...
        private void Plot()
        {
            ...
            foreach (var loc in location)
            {
                ...
                point = convert3D(objectX, objectY, objectZ, panX, panY, panZ, centerX, centerY, cameraX, cameraY,
                    cameraZ,
                    zoom);
                if (point.X!=0  &&point.Y != 0)
                {
                    ...
                    // dot.originX = stage.Width / 2;
                    //  dot.originY = stage.Height / 2;
                    ...
                    //   drawCanvas.Margin = new Thickness(200+(point.X),200- (point.Y), 0, 0);

                    //  drawCanvas = dot.Draw(color, 16);
                    ...
                    circle.Margin = new Thickness(400 + point.X, 400 - point.Y, 0, 0);
